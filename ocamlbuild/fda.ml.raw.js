var Options=require("Options");
var List=require("List");
var Printf=require("Printf");
var Pathname=require("Pathname");
var Hygiene=require("Hygiene");
var Log=require("Log");
var Exit_hygiene_failed="unknown primitive:caml_set_oo_id";
var
 laws=
  [0,
   [0,
    "Leftover OCaml compilation files",
    [0,
     [1,".cmo"],
     [0,[1,".cmi"],[0,[1,".cmx"],[0,[1,".cma"],[0,[1,".cmxa"],0]]]]],
    1],
   [0,
    [0,"Leftover OCaml type annotation files",[0,[1,".annot"],0],0],
    [0,
     [0,
      "Leftover object files",
      [0,
       [1,".o"],
       [0,
        [1,".a"],
        [0,[1,".so"],[0,[1,".obj"],[0,[1,".lib"],[0,[1,".dll"],0]]]]]],
      1],
     [0,
      [0,
       "Leftover ocamlyacc-generated files",
       [0,[0,".mly",".ml"],[0,[0,".mly",".mli"],0]],
       1],
      [0,
       [0,"Leftover ocamllex-generated files",[0,[0,".mll",".ml"],0],1],
       [0,
        [0,
         "Leftover dependency files",
         [0,[1,".ml.depends"],[0,[1,".mli.depends"],0]],
         1],
        0]]]]]];
var
 inspect=
  function(entry)
   {var $js=Log["dprintf"];
    $js(5,[0,[11,"Doing sanity checks",0],"Doing sanity checks"]);
    var evil=[0,0];
    var $js$1=Hygiene["check"];
    var $js$2=Options["sanitize"][1];
    if($js$2)
     {var $js$3=Pathname["concat"];
      var
       $js$4=
        $js$3(Options["build_dir"][1],Options["sanitization_script"][1]);
      var $js$5=[0,$js$4]}
    else
     {var $js$5=0}
    var stuff=$js$1($js$5,laws,entry);
    var $js$6=stuff;
    if($js$6)
     {var $js$7=List["iter"];
      $js$7
       (function(param)
         {var law=param[1];
          var $js$8=Printf["printf"];
          var match=law[3];
          var $js$9=match!=0;
          if($js$9)
           {var $js$10=!evil[1];
            if($js$10)
             {var $js$11=Printf["printf"];
              $js$11
               ([0,
                 [11,
                  "IMPORTANT: I cannot work with leftover compiled files.\n",
                  [10,0]],
                 "IMPORTANT: I cannot work with leftover compiled files.\n%!"]);
              evil[1]=1,0}
            else
             {}
            var $js$12="ERROR"}
          else
           {var $js$12="Warning"}
          $js$8
           ([0,[2,0,[11,": ",[2,0,[11,":\n",0]]]],"%s: %s:\n"],$js$12,law[1]);
          var $js$13=List["iter"];
          return $js$13
                  (function(msg)
                    {var $js$14=Printf["printf"];
                     return $js$14([0,[11,"  ",[2,0,[12,10,0]]],"  %s\n"],msg)},
                   param[2])},
        stuff);
      var $js$8=evil[1];
      if($js$8){return "unknown primitive:raise"}else{return 0}}
    else
     {return 0}};
module["exports"]=
{"Exit_hygiene_failed":Exit_hygiene_failed,"inspect":inspect};
