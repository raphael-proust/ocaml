...
module rec
  Compile_of_env:sig
                   type key =
                     ...
                     | CamlPrimitive of Primitive.description* J.expression
                     list
  ...
  struct
    ...
    let get_name (serializable_sigs : Types.signature_item list) (pos : int)
      =
      let ident = name_of_signature_item @@ (List.nth serializable_sigs pos) in
      ...
      (match key with
       | GetGlobal ((id : Ident.t),(pos : int),env) ->
           let v =
             ...
             | exception Not_found  ->
                 (match (Env.find_module (Pident id) env : Types.module_declaration)
                  with
             ...
             (let sigs =
              ...
              then
                ...
                E.arr ((E.int ~comment:(id.name) 0) ::
                  (Jlist.init len
                     (fun i  -> E.access (E.var id) (E.str (get_name sigs i)))))
              else E.var id)
    ...
    let required_modules (extras : Ident.t list) =
      let require (id : Ident.t) =
        (((E.call (E.js_var "require")
             [E.str (Printf.sprintf "./%s.js" (String.uncapitalize id.name))]),
           J.N) : (E.t* J.location)) in
      let referred =
        Hashtbl.fold
          (fun (id : Ident.t)  ->
             fun _  ->
               fun block  -> (S.variable id ~loc_exp:(require id)) :: block)
          cached_tbl ([] : J.block) in
      let forced =
        Jlist.filter_map
          (fun id  ->
         ...
         let compile_primitive (({ env;_} as st) : Compile_defs.cxt)
           ...
           (match prim with
            ...
            | Pctconst ct ->
                (match ct with
                 | Big_endian  ->
                     if Sys.big_endian then E.true_ else E.false_
         ...
         let rec compile_const (x : Lambda.structured_constant) =
           (match x with
            ...
            struct
              ...
              let dummy_output =
                { value = None; block = []; finished = false }
              let rec is_pure (lam : Lambda.lambda) =
                ...
                | _ -> false
              let handle_name_tail name should_return lam
                (exp : J.expression) =
              ...
              let handle_block_return st should_return lam block exp =
                (match (st, should_return) with
                 ...
                 | (Assign n,false ) ->
                     make_output (block @ [S.assign n exp])
                 | ((Declare _|Assign _),true ) ->
                     make_output [S.unknown_lambda lam] ~finished:true
                 | (EffectCall ,false ) -> make_output block ~value:exp
                 | (EffectCall ,true ) ->
                     make_output (block @ [S.return (Some exp)])
                       ~finished:true
              ...
              let exports (idents : Ident.t list) (lams : J.expression list)
                ...
                (let properties: J.property_map =
                   List.map2
                     (fun (i : Ident.t)  ->
                        fun (e : J.expression)  -> ((J.PNS (i.name)), e))
                     idents lams in
                 S.exp
              ...
              let break_of_output (x : output) =
                (match x with
                 ...
                 | { block; value = opt;_} ->
                     block @ [statement_of_opt_expr opt; S.break ()] : 
                J.block)
              let append (x : output) (y : output) =
                (match (x, y) with
                 | ({ finished = true ;_},y) -> x
                 | ({ block = []; value = None ;_},y) -> y
                 | ({ block = []; value = Some _;_},{ block = [];
                                                      value = None ;_})
                     -> x
              ...
              let dump_output ({ block; value = exp } : output)
                (oc : out_channel) =
                let cxt = Pp_js.empty in
                ...
                ignore (Pp_js.program cxt p [statement_of_opt_expr exp])
                 ...
                 J_helper:sig
                            ...
                            sig
                              ...
                              val obj :
                                ?comment:string -> J.property_map -> t
                              val true_ : t
                              val false_ : t
                              val unknown_lambda :
                            ...
                            sig
                              ...
                              val assign :
                                ?comment:string ->
                                  ?loc:J.location ->
                                    J.ident -> J.expression -> t
                              val assign_unit :
                                ?comment:string ->
                                  ?loc:J.location -> J.ident -> t
                              val declare_unit :
                                ?comment:string ->
                                  ?loc:J.location -> J.ident -> t
                              val while_ :
                   ...
                   module Exp =
                     struct
                       ...
                       let access ?comment  e0 e1 =
                         ...
                         t)
                       let var ?comment  id =
                         ({ expression_desc = (EVar id); comment } : 
                         t)
                       let str ?comment  s =
                       ...
                       let undefined ?comment  () =
                         js_var ?comment "undefined"
                       let math ?comment  v =
                   ...
                   module Stmt =
                     struct
                       ...
                       let mk ?comment  ?(loc= J.N)  statement_desc =
                         ({ J.loc = loc; statement_desc; comment } : 
                         t)
                       let comment ?(loc= J.N)  str =
                         ({
                            loc;
                            statement_desc = (Comment str);
                            comment = None
                          } : t)
                       let empty ?comment  ?(loc= J.N)  () =
                         ({ loc; statement_desc = Empty; comment } : 
                         t)
                       ...
                       let if_ ?comment  ?(loc= J.N)  ?else_  e then_ =
                         ({
                            loc;
                            statement_desc = (If (e, then_, else_));
                            comment
                          } : t)
                       ...
                       let exp ?comment  ?(loc= J.N)  e =
                         ({ loc; statement_desc = (Exp e); comment } : 
                         t)
                       let switch ?comment  ?(loc= J.N)  ?(others= []) 
                         ?default  e clauses =
                         ({
                            loc;
                            statement_desc =
                       ...
                       let return ?comment  ?(loc= J.N)  e =
                         ...
                         t)
                       let variable ?comment  ?(loc= J.N)  ?loc_exp 
                         (v : Ident.t) =
                         ({
                       ...
                       let while_ ?comment  ?(loc= J.N)  e st =
                         ...
                         t)
                       let for_ ?comment  ?(loc= J.N)  ?pred  ?step  init
                         block =
                         ({
                            loc;
                       ...
                       let unknown_lambda ?(comment= "unknown")  ?loc 
                         ...
                         (exp @@
                            (Exp.str ~comment
                               (Lambda_util.string_of_lambda lam)) : 
                         t)
                       ...
                       let break ?comment  ?label  ?(loc= J.N)  () =
                         ({ statement_desc = (J.Break label); loc; comment } : 
                         t)
                     end
                 end and
                      Compile_lambda:sig
                      ...
                      struct
                        ...
                        let add_jmps
                          (ls :
                            (Compile_defs.jbl_label* Compile_defs.value) list)
                          m =
                          List.fold_left
                            (fun acc  ->
                               fun (l,s)  ->
                                 Compile_defs.HandlerMap.add l s acc) m ls
                        ...
                        let rec compile_lambda
                          ...
                          (match lam with
                           | Lvar id ->
                               (Gen_util.handle_name_tail st should_return
                                  lam)
                                 @@ (E.var id)
                           | Lconst c ->
                               (Gen_util.handle_name_tail st should_return
                           ...
                           | Lprim (prim,args_lambda) ->
                               ...
                               let exp =
                                 Compile_primitive.compile_primitive cxt prim
                                   args_expr in
                               ...
                               (match compile_lambda
                                ...
                                | { block = b; value = Some e } ->
                                    (match (st, should_return,
                                     ...
                                     with
                                     | (NeedValue
                                        ,_,{ block = []; value = Some out1 },
                                        { block = []; value = Some out2 }) ->
                                         Gen_util.make_output b
                                           ~value:(E.econd e out1 out2)
                                     | (NeedValue ,_,_,_) ->
                           ...
                           | Lstaticcatch _ ->
                               ...
                               let code_jmps =
                                 List.map
                                   (fun (i,_,bindings)  ->
                                      (i,
                                        ({ exit_id; args = bindings } : 
                                        Compile_defs.value))) code_table in
                               let bindings =
                                 List.concat @@
                                   (List.map
                                      (fun (_,_,bindings)  -> bindings)
                                      code_table) in
                               let aux (cxt : Compile_defs.cxt) =
                               ...
                               (match st with
                                | NeedValue  ->
                                    let v = Gen_util.gen () in
                                    let lbody =
                                      compile_lambda { cxt with jmp_table }
                                        body in
                               ...
                               (match compile_lambda
                                ...
                                | { block; value = Some e } ->
                                    ...
                                    (match (st, should_return) with
                                     | (Declare x,_) ->
                                         Gen_util.make_output
                                           (block @ [S.declare_unit x])
                                     | (Assign x,_) ->
                                         Gen_util.make_output
                                           (block @ [S.assign_unit x])
                                     | (EffectCall ,true ) ->
                                         Gen_util.make_output
                                           (block @ [S.return_unit ()])
                                           ~finished:true
                           ...
                           | Lapply (fn,args_lambda,_info) ->
                               (match compile_lambda
                                ...
                                | { block = fn_code; value = Some fn } ->
                                    ...
                                    Gen_util.handle_block_return st
                                      should_return lam (fn_code @ args_code)
                                      exp)
                        ...
                        let lambda_as_module raw env (sigs : Types.signature)
                          filename (lam : Lambda.lambda) =
                          let out =
                            open_out
                                ...
                                struct
                                  ...
                                  let rec json f =
                                    (function
                                     ...
                                     | `Float v ->
                                         let s =
                                           Js_pp_util.string_of_number v in
                                         Pp.string f s
                                     ...
                                     struct
                                       ...
                                       let rec eliminate_ref id =
                                         ...
                                         | Lapply (e1,el,loc) ->
                                             Lapply
                                               ((eliminate_ref id e1),
                                                 (List.map (eliminate_ref id)
                                                    el), loc)
                                         | Lfunction (kind,params,body) as
                                             lam ->
                                             if
                                               IdentSet.mem id
                                                 (free_variables lam)
                                         ...
                                         | Lprim
                                             ...
                                             Lassign
                                               (id, (eliminate_ref id e))
                                         | Lprim
                                             (Poffsetref delta,(Lvar v)::[])
                                             when Ident.same v id ->
                                             Lassign
                                               (id,
                                                 (Lprim
                                          ...
                                          struct
                                            ...
                                            let non_breaking_space st =
                                              ...
                                              then
                                                ...
                                                push st
                                                  non_breaking_space_token
                                            let space ?(indent= 0)  st =
                                              if st.compact
                                              then
                                                st.pending_space <- Some "\n"
                                              else
                                                push st (Break (" ", indent))
                                            ...
                                            let set_compact st v =
                                              st.compact <- v
                                            let set_needed_space_function st
                                              f = st.needed_space <- Some f
                                          end and
                                               Pp_js:sig
                                                       type cxt
                                                       val empty : cxt
                                                 ...
                                                 let rec need_paren l
                                                   ...
                                                   | ECall (e,_,_)|EAccess
                                                     (e,_)|EDot (e,_) ->
                                                       (l <= 15) &&
                                                         (need_paren 15 e)
                                                   | EVar _|EStr _|EArr _
                                                     |EBool _|ENum _|EQuote _
                                                     |ERegexp _|EUn _|ENew _
                                                       -> false
                                                   | EFun _|EObj _ -> true
