             ...
             fun block  ->
               ((J.Variable
                   [(id,
                      (Some
                         ((J.ECall
                             ((EVar (Ident.create_persistent "require")),
         ...
         let compile_primitive (prim : Lambda.primitive)
           ...
           (match prim with
            ...
            | Pduprecord (_,_)|Plazyforce |Pccall _|Pbittest |Pcvtbint 
              ...
              |Pint_as_pointer  -> Gen_util.expr_of_unknow_primitive prim : 
           J.expression)
           ...
           (match x with
            ...
            Gen_util:sig
                       val handle_name_tail :
                         J.ident option ->
                           bool -> Lambda.lambda -> J.expression -> J.block
                       [@@ocaml.doc
            ...
            struct
              ...
              let handle_name_tail name should_return lam
                ...
                (match (name, should_return) with
                 | (None ,false ) ->
                     if is_pure lam then [] else [((Exp exp), N)]
              ...
              let exports (idents : Ident.t list) =
                (let properties: J.property_map =
                   List.map
                     (fun (i : Ident.t)  -> ((J.PNS (i.name)), (J.EVar i)))
                     idents in
                 Exp
                   (EBin
                      (Eq,
                        (EAccess
                           ((EVar (Ident.create_persistent "module")),
              ...
              let return_unit: J.block = [((J.Return (Some (J.ENum 0.))), N)]
                 ...
                 J:sig
                     ...
                     and statement =
                       ...
                       | Empty
                       | Exp of expression
                       | If of expression* loc_statement* loc_statement
                       option
                       | Do_while of loc_statement* expression
                       | While of expression* loc_statement
                       | For of (expression option,variable_declaration list)
                       either* expression option* expression option*
                       loc_statement
                       | ForIn of (expression,variable_declaration) either*
                       expression* loc_statement
                       | Continue of label option
                       | Break of label option
                       | Return of expression option
                       | Labelled of label* loc_statement
                       | Switch of expression* case_clause list* block
                       option* case_clause list
                       | Throw of expression
                     ...
                     and variable_declaration =
                       (ident* (expression* location) option)
                 ...
                 struct
                   ...
                   and statement =
                     ...
                     | Return of expression option
                     | Labelled of label* loc_statement
                     | Switch of expression* case_clause list* block option*
                     case_clause list
                     | Throw of expression
                     | Try of block* (ident* block) option* block option
                     | Fn of ident* ident list* block* location
                          ...
                          [((J.Return (Some (J.ENum 0.))), N)]
                           ...
                           struct
                             ...
                             let add_jmps (ls : (jbl_label* value) list) m =
                               List.fold_left
                                 (fun acc  ->
                                    fun (l,s)  -> HandlerMap.add l s acc) m
                                 ls
                             type cxt =
                               {
                               name: J.ident option;
                               ...
                               (match x with
                                | Lstaticcatch (l,(code,bindings),handler) ->
                                    flat_catches ((code, handler, bindings)
                                      :: acc) l
                                | _ -> (acc, x) : ((int* Lambda.lambda*
                                                    Ident.t list) list*
                                                    Lambda.lambda))[@@ocaml.doc
                                                                    " assume outer is [Lstaticcatch] "]
                             exception Not_an_expression
                             let rec expr_of_lambda cxt (lam : Lambda.lambda)
                               =
                               (match lam with
                                | Lvar id -> J.EVar id
                                | Lconst c -> Gen_primitive.compile_const c
                                ...
                                | Lprim
                                    (Pfield n,(Lprim (Pgetglobal id,[]))::[])
                                    ->
                                    J.EAccess
                                      ((EVar id),
                                        (EStr
                                           ((Gen_of_env.get_string (id, n)
                                               cxt.env), `Utf8)))
                                | Lprim (prim,args_lambda) ->
                                    Gen_primitive.compile_primitive prim
                                      (List.map (expr_of_lambda cxt)
                                         args_lambda)
                                | _ -> raise Not_an_expression : J.expression)
                             [@@ocaml.doc
                               " TODO:\n    for expression generation, \n    name, should_return  is not needed,\n    only jmp_table and env needed\n "]
                             and compile_lambda
                               ({ name; should_return; jmp_table } as cxt)
                               (lam : Lambda.lambda) =
                               (match expr_of_lambda cxt lam with
                                | e ->
                                    Gen_util.handle_name_tail name
                                      should_return lam
                                      (expr_of_lambda cxt lam)
                                | exception Not_an_expression  ->
                                    (match lam with
                                     | Lvar _|Lfunction _|Lprim
                                       (Pfield _,(Lprim
                                        (Pgetglobal _,[]))::[]) ->
                                         Gen_util.handle_name_tail name
                                           should_return lam
                                     ...
                                     | Lprim (prim,args_lambda) ->
                                         ...
                                         let exp: J.expression =
                                           Gen_primitive.compile_primitive
                                             prim args in
                                         args_code @
                                           (Gen_util.handle_name_tail name
                                              should_return lam exp)
                                                           ...
                                                           | Some x ->
                                                               Some
                                                                 (((J.Block
                                                                    ...
                                                                    N) : 
                                                                 J.loc_statement)) : 
                                                       J.loc_statement option))),
                                                   N)])
                                     | Lswitch
                                         (lam,{ sw_numconsts; sw_consts;
                                         ...
                                         (compile_lambda
                                           ...
                                           (let aux (v : J.expression)
                                              ...
                                              | (i,lam1)::xs ->
                                                  [((J.If
                                                       ((EBin
                                                         ...
                                                         J.loc_statement
                                                           option))), J.N)] in
                                            let tag_v = Gen_util.gen () in
                                            let block_tag: J.block =
                                              [((let open J in
                                                   Variable
                                                     [(tag_v,
                                                        (Some
                                            ...
                                            if sw_numconsts = 0
                                            then
                                              aux (J.EVar v) sw_blocks
                                                default
                                            else
                                              if sw_numblocks = 0
                                              then
                                                block_tag @
                                                  (aux (EVar tag_v) sw_consts
                                                     default)
                                              else
                                                [((J.If
                                                     ((EBin
                                     ...
                                     | Lstaticcatch _ ->
                                         ...
                                         (compile_lambda
                                           ...
                                           [((J.Switch
                                              ...
                                              N)]
                                     | Lwhile (p,body) ->
                                         let pred_symbol = Gen_util.gen () in
                                         let pred_block =
                                           compile_lambda
                                             {
                                               cxt with
                                               name = (Some pred_symbol);
                                               should_return = false
                                             } p in
                                         let block =
                                           [((J.For
                                     ...
                                     | Lfor (id,start,finish,direction,body)
                                         ->
                                         let start_id = Gen_util.gen () in
                                         let finish_id = Gen_util.gen () in
                                         let block =
                                           (compile_lambda
                                              {
                                                cxt with
                                     ...
                                     | Ltrywith (lam,id,catch) ->
                                         let block = compile_lambda cxt lam in
                                         [((J.Try
                                              (block,
                                                (Some
                                                   (id,
                               ...
                               (match (lam : Lambda.lambda) with
                                | Lprim (Psetglobal id,biglambda::[]) ->
                                    (match flat [] biglambda with
                                     | (Nop ,Lprim
                                        (Pmakeblock (_,_),module_exports))::rest
                                         ->
                                         let export_idents =
                                           List.map
                                             (fun (t : Lambda.lambda)  ->
                             ...
                             let lambda_as_module raw env filename
                               ...
                               let out =
                                 open_out
                                   (filename ^
                                      (if raw then ".raw.js" else ".js")) in
                               let js = compile env lam in
                               (let open Pp_js in
                                  program empty (Pp.to_out_channel out)) js;
                               close_out out
                             let () =
                                        ...
                                        then "NaN"
                                          ...
                                          (let vint = int_of_float v in
                                           ...
                                           then
                                             ...
                                             (let s1 =
                                                Printf.sprintf "%.12g" v in
                                              if v = (float_of_string s1)
                                              then s1
                                              else
                                                (let s2 =
                                                   Printf.sprintf "%.15g" v in
                                       ...
                                       let rec json f =
                                         (function
                                          ...
                                          | `O l ->
                                              (Pp.start_group f 1;
                                               Pp.string f "{";
                                               property_name_and_value_list f
                                                 l;
                                               Pp.string f "}";
