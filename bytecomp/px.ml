    ...
    module Optimizer :
      ...
      struct
        ...
        let beta_reduce params body args =
          List.fold_left2
            (fun l  -> fun param  -> fun arg  -> Llet (Strict, param, arg, l))
            body params args
        let simplify_lets (export_defs : Ident.t list) lam =
          ...
          let rec count bv =
            ...
            | Lapply (Lfunction (Curried ,params,body),args,_) when
                optimize && ((List.length params) = (List.length args)) ->
            ...
            | Lletrec (bindings,body) ->
                (List.iter (fun (v,l)  -> count bv l) bindings; count bv body)
          ...
          (let subst = Hashtbl.create 83 in
           let mklet (kind,v,e1,e2) =
             ...
             | Lvar w when
                 optimize &&
                   ((Ident.same v w) && (not (List.mem v export_defs)))
                 -> e1
           ...
           let rec simplif =
             ...
             | Llet (str,v,Lvar w,l2) when
                 optimize && (not (List.mem v export_defs)) ->
             ...
             | Llet (Alias ,v,l1,l2) ->
                 (match count_var v with
                  | 0 -> simplif l2
                  | 1 when optimize && (not (List.mem v export_defs)) ->
      ...
      struct
        ...
        let rec get_arity (tbl : (Ident.t,function_arities ref) Hashtbl.t)
          ...
          (match lam with
                    ...
                    let rec take (xs : int list) arg_length =
                      ...
                      | x::xs ->
                          ...
                          then xs
                            ...
                            else (x - arg_length) :: xs
                      | _ -> failwith (Lambda_util.string_of_lambda lam) in
                    try Approximate (b, (take xs (List.length args)))
                    with _ -> NA
               )
           | Lfunction (kind,params,l) ->
           ...
           | Lstringswitch (l,sw,d) ->
               (match d with
                | None  -> all_lambdas tbl (List.map snd sw)
                | Some v -> all_lambdas tbl (v :: (List.map snd sw)))
           | Lstaticraise _ -> NA
           | Lstaticcatch (_,_,handler) -> get_arity tbl handler
           | Ltrywith (l1,_,l2) -> all_lambdas tbl [l1; l2]
           | Lifthenelse (l1,l2,l3) -> all_lambdas tbl [l2; l3]
           | Lsequence (_,l2) -> get_arity tbl l2
           | Lsend (u,m,o,ll,v) -> NA
           | Levent (l,event) -> NA
           | Lifused (v,l) -> NA
          ...
          (let meta =
             {
               ...
               arities_tbl = (Hashtbl.create 31)
             } in
           let alias (k : Ident.t) (v : Ident.t) (kind : Lambda.let_kind) =
             (match Hashtbl.find meta.arities_tbl v with
              | exception Not_found  -> ()
              | v_arity -> Hashtbl.add meta.arities_tbl k v_arity);
             (match kind with
              | Alias  ->
                  if not @@ (List.mem k export_idents)
                  then Hashtbl.add meta.alias_tbl k v
              | Strict |StrictOpt |Variable  -> ()) in
           let annotate (k : Ident.t) (v : function_arities) =
             match Hashtbl.find meta.arities_tbl k with
             | exception Not_found  -> Hashtbl.add meta.arities_tbl k (ref v)
             | old -> old := v in
           let rec count (lam : Lambda.lambda) =
             ...
             | Llet (kind,k,Lvar v,l2) -> (alias k v kind; count l2)
             | Llet (str,v,(Lfunction (_,params,l) as u),body) ->
                 (annotate v (get_arity meta.arities_tbl u);
                  count l;
                  count body)
             | Llet (str,v,l1,l2) ->
                 (if List.mem v export_idents
                  then annotate v (get_arity meta.arities_tbl l1);
                  count l1;
                  count l2)
             | Lletrec (bindings,body) ->
                 (List.iter
                    (function
                     | x ->
                         (match x with
                          | (k,Lambda.Lvar v) -> alias k v Strict
                          | (k,l) ->
                              ((match l with
                                | Lfunction _ ->
                                    annotate k (get_arity meta.arities_tbl l)
        ...
        let simplify_alias export_idents (lam : Lambda.lambda) =
          (let { alias_tbl = tbl } as meta =
           ...
           let rec simpl (lam : Lambda.lambda) =
             ...
             | Lconst _ -> lam
             | Lvar v ->
                 (try Lvar (Hashtbl.find tbl v) with | Not_found  -> lam)
             ...
             | Lprim (Pfield i,(Lvar v)::[]) ->
                 (match Hashtbl.find tbl v with
                  ...
                  | g -> Lprim ((Pfield i), [Lprim ((Pgetglobal g), [])]))
        ...
        let compile ?filename  env sigs lam =
          (let exports = Translmod.get_export_identifiers () in
           ...
           let (lam,side_effects,meta) =
             (Alpha_pass.simplify_alias exports) @@
                ...
                | (Nop (Lprim (Pmakeblock (_,_,_),lambda_exports)))::rest ->
                    ...
                    let js: Gen_util.output =
                      ((((Gen_util.make_output
                           ...
                           ++
                           (Gen_util.make_output
                              (Compile_of_env.required_modules side_effects)))
