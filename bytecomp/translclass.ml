...
let mkappl (func, args) = Lapply (func, args, Lambda.default_apply_info ());;
...
let output_methods tbl methods lam =
  ...
  | _ ->
      lsequence (mkappl(oo_prim "set_methods",
                        [Lvar tbl; Lprim(Pmakeblock(0, Lambda.default_tag_info, Immutable), methods)]))
        lam
...
let transl_class_rebind ids cl vf =
  try
    ...
    let obj_init0 = lapply (Lvar obj_init) [Lvar self] (Lambda.default_apply_info ()) in
